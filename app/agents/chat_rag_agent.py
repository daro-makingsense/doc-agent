...
class ChatRAGAgent:
    ...
    def __init__(self):
        ...
        self.system_prompt = """You are an expert financial analyst and advisor with access to detailed financial documents. 
    ...
    async def _generate_response(self, state: ChatState) -> ChatState:
        ...
        try:
            ...
            if state.use_rag and state.context_used:
                user_message = f"""Based on the following information from the financial document:

{state.context_used}

User Question: {state.message}

Please provide a comprehensive answer based on the document information. If the document doesn't contain relevant information for the question, please state that clearly."""
            else:
