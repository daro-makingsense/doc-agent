...
@router.post("/research/start", response_model=schemas.ResearchResponse)
...
@router.get("/research/tasks", response_model=List[schemas.ResearchTask])
def list_research_tasks(
    ...
    try:
        ...
        if document_id:
            query = query.filter(ResearchTask.document_id == document_id)
...
@router.get("/research/tasks/{task_id}", response_model=schemas.ResearchTask)
def get_research_task(
    ...
    try:
        ...
        logger.error(f"Error getting research task {task_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# Chat endpoints
@router.post("/chat/send", response_model=schemas.ChatResponse)
...
@router.get("/chat/sessions/{session_id}/history", response_model=List[schemas.ChatMessage])
