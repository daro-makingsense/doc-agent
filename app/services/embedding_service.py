    ...
    async def embed_document(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                vectors_to_upsert = []
                for j, (embedding, metadata, chunk) in enumerate(zip(embeddings, batch_metadatas, batch_chunks)):
                    vector_id = self._generate_vector_id(document_id, chunk.id)
                ...
                with get_db_session() as db:
                    for j, chunk in enumerate(batch_chunks):
                        ...
                        chunk_record = db.query(DocumentChunk).filter(DocumentChunk.id == chunk.id).first()
                        if chunk_record:
